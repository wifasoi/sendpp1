...
class Comunication:
    ...
    def __init__(self, service: QLowEnergyService):
        self.service = service
        self.rx = self.service.characteristic(QBluetoothUuid(READ_CHAR_UUID))
        self.tx = self.service.characteristic(QBluetoothUuid(WRITE_CHAR_UUID))
    ...
    def receive(self, info, value):
        pass

    def machine_request(self, cmd: MachineCommand, data: bytearray = b'') -> bytearray:
        send_command = QState()
        receive_command = QState()


        self.service.writeCharacteristic(self.tx, self.build_cmd(cmd,data), QLowEnergyService.WriteMode.WriteWithResponse)
        self.service.characteristicWritten.connect(self.receive)
        self.service.readCharacteristic(self.rx)
        self.service.characteristicRead.connect(self.receive)
    


    @Slot()
    def clear_error(self) -> None:
        self.machine_request(MachineCommand.CLEAR_ERROR)
        logger.success("Error cleared")

    @Slot()
    def delete_emboridery(self):
        if not self.machine_request(MachineCommand.DELETE_EMBROIDERY):
    ...
    @Slot()
    def resume_emboridery(self) -> None:
        data = self.machine_request(MachineCommand.RESUME_EMBROIDERY)
        if len(data) > 0 and data[0] != 0:
    ...
    def start_emboridery(self) -> None:
        data = self.machine_request(MachineCommand.START_SEWING)
        logger.success("Starting embroidery completed successfully")
        #TODO: need to check the state change

    def send_layout(self, layout: EmbroideryLayout, mask: EmbroideryBoundingBox) -> None:
        self.machine_request(MachineCommand.LAYOUT_DATA, layout.to_bytes() + mask.to_bytes())
        logger.success("Sended layout data")

    def do_regular_inspection(self) -> None:
        data = self.machine_request(MachineCommand.REGULAR_INSPECTION)
        if data and data[0] != 1:
            logger.error("Regular inspection failed")
            return
        logger.success("Done regular inspection")

    def do_hoop_avoidance(self) -> None:
        self.machine_request(MachineCommand.HOOP_AVOIDANCE)
        logger.success("Sent Hoop avoidence command")

    def prepare_transfer(self, size: int, checksum: int) -> None:
        data = self.machine_request(MachineCommand.PREPARE_TRANSFER, b'\03' + size.to_bytes(length=2,byteorder='little') + checksum.to_bytes(length=2,byteorder='little'))
    ...
    def transfer(self, data: bytearray) -> None:
        ...
        checksum = sum(data)
        self.prepare_transfer(len(data),checksum)
        for index, chunk in enumerate(batched(data,n=transfer_size,strict=False)):
            chunk_checksum = sum(chunk)
            offset = index * len(chunk)
            self.command(MachineCommand.DATA_PACKET, offset.to_bytes(4,'little') + chunk + chunk_checksum.to_bytes(1,'little'))
        #TODO: Error handling
        while((result := self.receive()) > 3 and result[2] != 0):
            if result[2] == 2:
                asyncio.sleep(1)
...
class EmbroideryMachine:
    ...
    def send(self, cmd: bytearray, data: bytearray = b'', response=True) -> None:
        if tx_characteristic := self.service.characteristic(QBluetoothUuid(WRITE_CHAR_UUID)):
            write_type = QLowEnergyService.WriteMode.WriteWithResponse if response else QLowEnergyService.WriteMode.WriteWithoutResponse
            self.service.writeCharacteristic(tx_characteristic, self.build_cmd(cmd,data), write_type)
            logger.trace("BTSend: {}(0x{})", cmd.hex(), data.hex())

    def receive(self) -> bytearray:
        if rx_characteristic := self.service.characteristic(QBluetoothUuid(READ_CHAR_UUID)):
            self.service.readCharacteristic(rx_characteristic)
